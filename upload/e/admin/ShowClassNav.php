...
if($ecms==1)
{
	//$show="增加信息：<select name=\\\"select\\\" onchange=\\\"if(this.options[this.selectedIndex].value!=0){self.location.href='AddNews.php?".$ecms_hashur['ehref']."&bclassid=&classid='+this.options[this.selectedIndex].value+'&enews=AddNews';}\\\"><option value='0'>選擇增加信息的欄目</option>".$do_class."</select>";
...
elseif($ecms==2)
{
	$show="<select name='addclassid' id='addclassid'><option value='0'>選擇增加信息的欄目</option>".$do_class."</select>";
	echo"<script>parent.document.getElementById(\"showaddclassnav\").innerHTML=\"".$show."\";";

	$show="<select name='classid' id='classid'><option value='0'>所有欄目</option>".$do_class."</select>";
	echo"parent.document.getElementById(\"searchclassnav\").innerHTML=\"".$show."\";";

	$show="<select name='to_classid' id='to_classid'><option value='0'>選擇要移動/複製的目標欄目</option>".$do_class."</select>";
...
elseif($ecms==4)
{
	$show="<select name='searchclassid' id='searchclassid'><option value='all'>所有欄目</option>".$do_class."</select>";
