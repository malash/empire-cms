...
function DoRepNewstext($start,$oldword,$newword,$field,$classid,$tid,$tbname,$over,$dozz,$dotxt,$userid,$username){
	...
	CheckLevel($userid,$username,$classid,"repnewstext");//驗證權限
	//變量替換
	...
	if($dotype==1)
	{
		...
		//替換完畢
		insert_dolog("tbname=".$tbname."&field=".$field."&dotype=1<br>oldword=".$oldword."<br>newword=".$newword);//操作日誌
	...
	elseif($fr['savetxt'])
	{
		...
		while($r=$empire->fetch($sql))
		{
			...
			if($dozz==1)//正則
			{
				$newvalue=DoRepNewstextZz($repoldword,$repnewword,$value);//正則替換
			}
			else//普通
			{
		...
		if(empty($b))
		{
			insert_dolog("tbname=".$tbname."&field=".$field."<br>oldword=".$oldword."<br>newword=".$newword);//操作日誌
			printerror("DoRepNewstextSuccess","db/RepNewstext.php?tid=$tid".hReturnEcmsHashStrHref2(0));
		}
	...
	elseif($dozz==1)
	{
		...
		while($r=$empire->fetch($sql))
		{
			...
			if($fr['tbdataf'])//副表
			{
				...
				$infodatatb=ReturnInfoDataTbname($tbname,$r['checked'],$infor['stb']);
				$finfor=$empire->fetch1("select ".$field." from ".$infodatatb." where id='$r[id]' limit 1");
				$newvalue=DoRepNewstextZz($oldword,$newword,stripSlashes($finfor[$field]));//正則替換
				$empire->query("update ".$infodatatb." set ".$field."='".addslashes($newvalue)."' where id='$r[id]'");
			}
			else//主表
			{
				$newvalue=DoRepNewstextZz($oldword,$newword,stripSlashes($infor[$field]));//正則替換
...
function ClearBreakInfo($add,$userid,$username){
	global $empire,$dbtbpre;
	CheckLevel($userid,$username,$classid,"changedata");//驗證權限
	$tid=(int)$add['tid'];
	if(!$tid)
	{
...
function ResetAddDataNum($type,$from,$userid,$username){
	...
	if($type=='info')
	{
		//CheckLevel($userid,$username,$classid,"info");//驗證權限
	}
	elseif($type=='pl')
	{
		//CheckLevel($userid,$username,$classid,"pl");//驗證權限
	}
	else
	{
		printerror("ErrorUrl","");
