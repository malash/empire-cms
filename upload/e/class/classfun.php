...
function AddZt($add,$userid,$username){
	...
	CreateZtPath($add[ztpath]);//建立專題目錄
	$addtime=time();
	//取得表名
	...
	GetClass();//更新緩存
	if($sql){
		insert_dolog("ztid=".$ztid."<br>ztname=".$add[ztname]);//操作日誌
...
function EditZt($add,$userid,$username){
	...
	if($add[oldztpath]<>$add[ztpath]){
		...
		if($add['oldpripath']==$add['pripath']){
			$new="../../";
			@rename($new.$add[oldztpath],$new.$add[ztpath]);//改變目錄名
		}
		else{
			CreateZtPath($add[ztpath]);//建立專題目錄
		}
	...
	if($sql)
	{
		...
		insert_dolog("ztid=".$add[ztid]."<br>ztname=".$add[ztname]);//操作日誌
...
function DelZt($ztid,$userid,$username){
	...
	GetClass();//更新緩存
	if($sql){
		insert_dolog("ztid=".$ztid."<br>ztname=".$r[ztname]);//操作日誌
...
function SaveTogZtInfo($add,$userid,$username){
	...
	if($sql)
	{
		insert_dolog("togid=$togid&togztname=$add[togztname]");//操作日誌
...
function DelTogZtInfo($add,$userid,$username){
	...
	if($sql)
	{
		insert_dolog("togid=$togid&togztname=$r[togztname]");//操作日誌
...
function AddWbClass($add,$userid,$username){
	...
	if($sql)
	{
		...
		insert_dolog("classid=".$lastid."<br>classname=".$add[classname]);//操作日誌
...
function EditWbClass($add,$userid,$username){
	...
	if($sql)
	{
		insert_dolog("classid=".$add[classid]."<br>classname=".$add[classname]);//操作日誌
		if($updatecache)
		{
			//刪除導航緩存
			$empire->query("delete from {$dbtbpre}enewsclassnavcache where navtype='listclass'");
...
function EditClass($add,$userid,$username){
	...
	if($add['oldclasspath']<>$add['pripath'].$add['oldcpath'])//更換父欄目
	{
	...
	if($add[oldclasspath]<>$classpath&&$checkclasspath==$add['oldcpath']){
		if(file_exists("../../".$classpath)){//檢測目錄是否存在
			printerror("ReClasspath","");
		}
    }
	//取得表名
	...
	{
		if($add[modid]<>$add[oldmodid])//換系統模型
		{
	...
	if($add[bclassid]<>$add[oldbclassid]||($add[oldclasspath]<>$classpath&&$add['classpath']==$add['oldcpath'])){
		...
		if($add['oldcpath']<>$add['classpath'])//更換欄目目錄
		{
...
function DelClass($classid,$userid,$username){
	...
	$cache_mid=$r[modid];
	$cache_url="CreateCache.php?enews=$cache_enews&mid=$cache_mid&ecmstourl=$cache_ecmstourl&mess=$cache_mess".hReturnEcmsHashStrHref2(0);
	insert_dolog("classid=".$classid."<br>classname=".$r[classname]);//操作日誌
