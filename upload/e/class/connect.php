...
function ReplaceListVars($no,$listtemp,$subnews,$subtitle,$formatdate,$url,$haveclass=0,$r,$field,$docode=0){
	...
	for($i=1;$i<$fcount;$i++)
	{
		...
		elseif(strstr($emod_r[$mid]['smalltextf'],','.$f.','))//簡介
		{
...
function DoTranUrl($url,$classid){
	...
	elseif(strstr($ecms_config['sets']['mediaplayertype'],','.$r[filetype].',')||strstr($ecms_config['sets']['realplayertype'],','.$r[filetype].','))//多媒體
	{
		$r[type]=3;
	}
	else
...
function ReturnDSofturl($downurl,$qz,$path='../../',$isdown=0){
	...
	$url=$urlr['repath'];
	@include_once(ECMS_PATH."e/DownSys/class/enpath.php");//防盜鏈
	if($isdown)
	{
...
function DoSafeCheckFromurl(){
	global $ecms_config;
	if($ecms_config['esafe']['ckfromurl']==0||defined('EmpireCMSNFPage'))//不啟用
	{
		return '';
	}
	...
	if($ecms_config['esafe']['ckfromurl']==1)//全部啟用
	{
	...
	elseif($ecms_config['esafe']['ckfromurl']==2)//後台啟用
	{
	...
	elseif($ecms_config['esafe']['ckfromurl']==3)//前台啟用
	{
		if(!defined('EmpireCMSAdmin')&&!stristr($fromurl,$domain))
		{
			echo"";
