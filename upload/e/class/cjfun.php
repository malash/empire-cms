...
function CjNewsIn($classid,$id,$checked,$uptime,$userid,$username){
	...
	CheckLevel($userid,$username,$classid,"cj");//操作權限
	$count=count($id);
	if(empty($count))
	{
...
function ReturnCJ_str($text,$exp,$info){
	...
	$rep="[!--".$exp."--]";
	//替換
	$num=CountCJ_site($text,$exp);//取得符號位置
...
function CjFormatNewstime($newstime){
	$newstime=str_replace(array('年','月','日','.','時','分','秒','/'),array('-','-','','-',':',':','','-'),$newstime);
	return $newstime;
}
	...
	while($nr=$empire->fetch($sql))
	{
		...
		for($c=0;$c<$count-1;$c++)
		{
			...
			if($dofield=="newstime")
			{
				...
				{
					$newstime=CjFormatNewstime($newstime);//時間轉換
				}
				continue;
			}
			//------------------------------信息來源
...
function LoadOutCj($classid,$userid,$username){
	...
	CheckLevel($userid,$username,$classid,"loadcj");//驗證權限
	$classid=(int)$classid;
	if(!$classid)
	{
