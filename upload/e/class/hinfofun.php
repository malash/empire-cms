...
function AddInfoVote($classid,$id,$add){
	...
	if($num)	//修改
	{
		$votetext=ReturnVote($add['vote_name'],$add['vote_num'],$add['delvote_id'],$add['vote_id'],1);	//返回組合
	...
	{
		$votetext=ReturnVote($add['vote_name'],$add['vote_num'],$add['delvote_id'],$add['vote_id'],0);	//返回組合
...
function AddNews($add,$userid,$username){
	...
	$add=DoPostInfoVar($add);//返回變量
	$ret_r=ReturnAddF($add,$class_r[$add[classid]][modid],$userid,$username,0,0,1);//返回自定義字段
	$newspath=FormatPath($add[classid],$add[newspath],1);//查看目錄是否存在，不存在則建立
	//審核權限
	...
	if($sql)
	{
		...
		insert_dolog("classid=$add[classid]<br>id=".$id."<br>title=".$add[title],$pubid);//操作日誌
...
function EditNews($add,$userid,$username){
	...
	$doselfinfo=CheckLevel($userid,$username,$add[classid],"news");//操作權限
	...
	if($doselfinfo['doselfinfo']&&($checkr['userid']<>$userid||$checkr['ismember']))//只能編輯自己的信息
	{
	...
	$ret_r=ReturnAddF($add,$class_r[$add[classid]][modid],$userid,$username,1,0,1);//返回自定義字段
	$deloldfile=0;
	if($add[groupid]<>$add[oldgroupid]||($index_checkr['checked']&&!$add[checked]))//改變文件權限
	{
        DelNewsFile($checkr[filename],$checkr[newspath],$add[classid],$add[$pf],$add[oldgroupid]);//刪除舊的文件
		$deloldfile=1;
	}
	//簽發
	...
	{
		$add[newspath]=FormatPath($add[classid],$add[newspath],1);//查看目錄是否存在，不存在則建立
		$updatefile.=",newspath='$add[newspath]'";
		if($deloldfile==0)
		{
			DelNewsFile($checkr[filename],$checkr[newspath],$add[classid],$add[$pf],$add[oldgroupid]);//刪除舊文件
			$deloldfile=1;
		}
	...
	if($sql)
	{
		...
		insert_dolog("classid=$add[classid]<br>id=".$add[id]."<br>title=".$add[title],$pubid);//操作日誌
...
function DelNews($id,$classid,$userid,$username){
	...
	$doselfinfo=CheckLevel($userid,$username,$classid,"news");//操作權限
	if(!$doselfinfo['dodelinfo'])//刪除權限
	{
	...
	if($doselfinfo['doselfinfo']&&($r[userid]<>$userid||$r[ismember]))//只能編輯自己的信息
	{
		printerror("NotDoSelfinfo","history.go(-1)");
    }
	$pubid=ReturnInfoPubid($classid,$id);
	...
	DelNewsFile($r[filename],$r[newspath],$classid,$r[$pf],$r[groupid]);//刪除信息文件
	...
	if($sql)
	{
		...
		insert_dolog("classid=$classid<br>id=".$id."<br>title=".$r[title],$pubid);//操作日誌
...
function AddInfoToReHtml($classid,$dore){
	...
	insert_dolog("classid=".$classid."<br>do=".$dore);//操作日誌
...
function hReClassHtml($sonclass){
	...
	for($i=1;$i<$count-1;$i++)
	{
		...
		elseif($class_r[$r[$i]]['islist']==3)//欄目綁定信息
		{
			ReClassBdInfo($r[$i]);
		}
		else//父欄目
...
function DelInfoToCopyInfo($classid,$id,$r,$userid,$username,$usergroupr){
	...
	for($i=1;$i<$count-1;$i++)
	{
		...
		DelNewsFile($infor[filename],$infor[newspath],$infor[classid],$infor[$pf],$infor[groupid]);//刪除信息文件
...
function TopNews_all($classid,$id,$istop,$userid,$username){
	...
	$doselfinfo=CheckLevel($userid,$username,$classid,"news");//驗證權限
	if(!$doselfinfo['dogoodinfo'])//權限
	{
...
function DoWfInfo($add,$userid,$username){
	...
	if($doing==1)//通過
	{
		if($wfitemr[lztype]==0)//普簽
		{
			if($wfitemr['tno']=='100')//全部通過
			{
...
function DelInfoData($start,$classid,$from,$retype,$startday,$endday,$startid,$endid,$tbname,$add,$userid,$username){
	...
	if($classid)
	{
		if(empty($class_r[$classid][islast]))//大欄目
		{
	...
	if($ismember)
	{
		...
		elseif($ismember==2)//會員+用戶
		{
			$add1.=" and userid>0";
		}
		elseif($ismember==3)//會員
		{
			$add1.=" and userid>0 and ismember=1";
		}
		elseif($ismember==4)//用戶
		{
...
function DoDocInfo($tb,$r,$ecms=0){
	...
	if($ecms==1)//還原
	{
		...
		$ytable=$dbtbpre.'ecms_'.$tb.'_index';	//目標索引表
		$ytable1=$dbtbpre.'ecms_'.$tb;	//目標主表
		$ytable2=$dbtbpre.'ecms_'.$tb.'_data_'.$r[stb];	//目標副表
	}
	else//歸檔
	{
		...
		$ytable=$dbtbpre.'ecms_'.$tb.'_doc_index';	//目標索引表
		$ytable1=$dbtbpre.'ecms_'.$tb.'_doc';	//目標主表
		$ytable2=$dbtbpre.'ecms_'.$tb.'_doc_data';	//目標副表
	}
	...
	$r=array_merge($r,$fr);
	$ret_r=ReturnAddF($r,$mid,$userid,$username,10,0,0);//返回自定義字段
	//索引表
...
function DoDocCkInfo($tb,$where,$ecms=0){
	...
	if($ecms==1)//還原
	{
		...
		$table1=$dbtbpre.'ecms_'.$tb.'_doc';	//主表
		$table2=$dbtbpre.'ecms_'.$tb.'_doc_data';	//副表
		$ytable=$dbtbpre.'ecms_'.$tb.'_index';	//目標主表
		$ytable1=$dbtbpre.'ecms_'.$tb.'_check';	//目標主表
		$ytable2=$dbtbpre.'ecms_'.$tb.'_check_data';	//目標副表
	}
	else//歸檔
	{
		...
		$ytable=$dbtbpre.'ecms_'.$tb.'_doc_index';	//目標主表
		$ytable1=$dbtbpre.'ecms_'.$tb.'_doc';	//目標主表
		$ytable2=$dbtbpre.'ecms_'.$tb.'_doc_data';	//目標副表
	}
	//轉換副表
...
function DoInfoSendNotice($userid,$username,$to_userid,$to_username,$causetext,$infor,$ecms=0){
	...
	$msgtext="您發佈的信息被 <strong>$dousername</strong> 執行 <strong>$doing</strong> 操作<br>
<br>
<strong>信息標題：</strong><a href='".$titleurl."'>".$infotitle."</a><br>
<strong>發佈時間：</strong>".$infotime."<br>
<strong>所在欄目：</strong><a href='".$classurl."'>".$classname."</a><br>
<strong>操作時間：</strong>$dotime<br>
