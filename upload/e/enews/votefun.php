...
function AddVote($voteid,$vote){
	...
	if($sql)
	{
		esetcookie("lastvotetime",time(),time()+3600*24);//砞竚程祇丁
...
function AddInfoVote($classid,$id,$vote){
	...
	if($sql)
	{
		esetcookie("lastivotetime",time(),time()+3600*24);//砞竚程祇丁
...
function DoVote($r,$vote){
	...
	$new_votetext=substr($new_votetext,0,strlen($new_votetext)-2);//奔程才
	//计舱
...
function AddInfoPfen($add){
	...
	if($sql)
	{
		esetcookie('lastforfenid',$classid.'n'.$id,time()+30*24*3600);	//程祇
		if($doajax==1)
		{
