...
function AddVote($voteid,$vote){
	...
	if($sql)
	{
		esetcookie("lastvotetime",time(),time()+3600*24);//設置最後發表時間
...
function AddInfoVote($classid,$id,$vote){
	...
	if($sql)
	{
		esetcookie("lastivotetime",time(),time()+3600*24);//設置最後發表時間
...
function DoVote($r,$vote){
	...
	$new_votetext=substr($new_votetext,0,strlen($new_votetext)-2);//去掉最後的字符
	//返回數組
...
function AddInfoPfen($add){
	...
	if($sql)
	{
		esetcookie('lastforfenid',$classid.'n'.$id,time()+30*24*3600);	//最後發佈
		if($doajax==1)
		{
